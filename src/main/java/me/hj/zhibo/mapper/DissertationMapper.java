...
public interface DissertationMapper extends BaseMapper<Dissertation> {
    @Select("select d.*, i.name as teacher from tb_dissertation d left join tb_user_info i on d.uid=i.uid")
    ...
    @Select("SELECT d.*,i.name as teacher FROM (tb_dissertation d LEFT JOIN tb_disser_stu s ON d.did=s.did) LEFT JOIN tb_user_info i ON d.uid=i.uid WHERE s.uid=#{uid}")
    DissertationVO myAspiration(int uid);

    @Update("update tb_dissertation SET `status`=0 WHERE did=#{did}")
    int updateStatus(int did);

    @Select("SELECT d.*,i.name as teacher FROM tb_dissertation d LEFT JOIN tb_user_info i ON d.uid=i.uid WHERE d.name LIKE #{words}")
